...
- 公開版では、初期状態から `desc` が未入力の語も表示します。設定で非表示に切り替えられます。
...
- 既存データに `links` がない場合、Wikipediaは表示Onとして扱います。Wikipediaを未確認の語はローカルエディタで表示をOffにでき、公開画面ではWikipediaボタンを無効状態で表示します。
...
音量設定はブラウザに保存され、次回表示時に復元されます。初期音量は10%、音声はOnです。音声のOn/Offと音源切り替えにはフェードが入ります。
